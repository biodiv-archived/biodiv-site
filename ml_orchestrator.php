...
function __autoload($class_name) {
  ...
  if ($found !== FALSE) {
      ...
      $found = stream_resolve_include_path('Class_' . $class_name . '.php' !== FALSE);
      if($found !== FALSE) { 
          include $found;
      }
  }
...
switch ($action) {
  case 'getStatistics':
    print '{';
    $query='select count(*) from "Meta_Layer" where status = 1';
    $result=db_query($query);
    foreach(db_fetch_array($result) as $row) {
        print '"layer_count":'.$row;
    }
    print ',';
    $query='select count(*) from node where type=\'checklist\';';
    $result=db_query($query);
    foreach(db_fetch_array($result) as $row) {
        print '"checklist_count":'.$row;
    }
    print ',';
    $server = $_SERVER['SERVER_NAME'];
    $url = 'http://'.$server.':8080/biodiv/species/count';
    $arr=file($url) ;
    print '"species_page_count":'.'1083';
    print ',';
    $query='select count(*) from occurrence;';
    $result=db_query($query);
    foreach(db_fetch_array($result) as $row) {
        print '"occurrence_count":'.$row;
    }
    print ',';
    $query='select count(distinct species_name) from occurrence;';
    $result=db_query($query);
    foreach(db_fetch_array($result) as $row) {
        print '"species_occurrence_count":'.$row;
    }
    print '}';
