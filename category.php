...
$wg_layers = implode(",", array(173,175,37,7,6,36,41,35,39,32,33,34,61,60,160,4,46,48,89,2,49,50,63,62,64,1,42,167,45,8,5,43,3,38,44,47,126,125,127,185,201,203,207,209));
...
EOF;
  ...
  (SELECT * FROM "Theme" where theme_id = parent_id and theme_type = %d and status = 1 order by theme_name) as a
  left join
  (select theme_id, count(*) as layer_count from "Theme_Layer_Mapping", "Meta_Layer" where "Theme_Layer_Mapping".layer_id = "Meta_Layer".layer_id and "Meta_Layer".status = 1 and "Theme_Layer_Mapping".layer_id in (173,175,37,7,6,36,41,35,39,32,33,34,61,60,160,4,46,48,89,2,49,50,63,62,64,1,42,167,45,8,5,43,3,38,44,47,126,125,127,185,201,203,207,209) group by theme_id) as b
    on a.theme_id = b.theme_id
EOF;
	break;
}
...
function getLayersForTheme($theme_id) {
  ...
  switch ($_SERVER['SERVER_NAME']){	
	...
	case "thewesternghats.org":
	case "westernghatsportal.org":
  		$query = "SELECT layer_tablename, layer_name, access, p_nid, participation_type,layer_type FROM \"Meta_Layer\" where layer_id in( select layer_id from \"Theme_Layer_Mapping\" where theme_id = %d) and layer_id in(173,175,37,7,6,36,41,35,39,32,33,34,61,60,160,4,46,48,89,2,49,50,63,62,64,1,42,167,45,8,5,43,3,38,44,47,126,125,127,185,201,203,207,209) and status = 1 order by layer_name;";
		break;
  }
...
function getParticipatoryLayers() {
  switch ($_SERVER['SERVER_NAME']){
	...
	case "thewesternghats.org":
	case "westernghatsportal.org":
  		$query = 'select layer_tablename, layer_name, access, p_nid, participation_type,layer_type  from "Meta_Layer" where participation_type in (1,2, 3) and layer_id in(173,175,37,7,6,36,41,35,39,32,33,34,61,60,160,4,46,48,89,2,49,50,63,62,64,1,42,167,45,8,5,43,3,38,44,47,126,125,127,185,201,203,207,209) and status = 1 order by layer_name;';
                break;
  }
